...
seg_points0 = [[15, 50], [371, 600], [657, 900], [1121, 1365], [1414, 1800], [2036, 2230], [2292, 2700], [2828, 3138]]  # c21_0625 music segment points

seg_points1 = [[0, 14], [51, 370], [601, 656], [901, 1120], [1366, 1413], [1801, 2035], [2231, 2291], [2701, 2827]]  # c21_0625 speech segment points

seg_points2 = [[0, 25], [320, 569], [620, 875], [1080, 1330], [1398, 1775], [1989, 2264], [2295, 2675], [2838, 3093], [3143, 3570]]  # c21_0624 music segment points

seg_points3 = [[25, 320], [569, 620], [875, 1080], [1330, 1398], [1775, 1989], [2264, 2295], [2675, 2838], [3093, 3143], [3570, 3600]]  # c21_0624 speech segment points
...
def audio_segment(audio_url, seg_points):
    ...
    for seg_point in seg_points:
        ...
        filename = "%s_%s_%s.wav" % (audio_url.split('/')[-1].split('.')[0], start, end)
        # filename = "0629-%s.wav" %(count)
        filepath = 'segs/'
        segmented_audio = audio[int(start * 1000): int(end * 1000)]
        segmented_audio.export(filepath + filename + '_origin', format="wav")
        io = StringIO()
        io.write(u'ffmpeg -y -i {} -ar 8000 {}'.format(filepath + filename + '_origin', filepath + filename))
        cmd = io.getvalue()
